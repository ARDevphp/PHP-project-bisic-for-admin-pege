...
if (isset($_GET['id'])) {
    ...
    $news_row = getNewsById($id, 'texnika');
    $_SESSION['image'] = $news_row['image'];

    $marka_id = $news_row['marka_id'];
    $texcate = getAvtoCateTag('texcate', $marka_id);
...
if (isset($_SESSION['user'])) {
    ...
    <div class="container-fluid">
        <div class="row mt-4">
            <form method="post" action="ub_texo.php">
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <select name="mak" class="form-select" aria-label="Default select example">
                        <option selected value="<?=$texcate['id']?>"><?= $texcate['name'] ?></option>
                        <?php
                ...
                <button type="submit" name="tex" class="btn btn-primary">Update</button>
            </form>
        </div>
    </div>
